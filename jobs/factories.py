...
class DagFactory:
    ...
    def create_schedule_dag(
        *,
        ...
        with DAG(
            dag_id=f'{identifier}-schedule-dag',
            schedule=None,
            catchup=False,
            tags=tags,
            params=parameters,
            ...
            commands = ['python', 'schedule_puller.py', '-b', Variable.get(bucket)]
            ...
            template = PythonOperator(
                task_id='template-generator',
                python_callable=helpers.publish_template,
                op_kwargs=args,
            )

            k8_job = KubernetesJobOperator(
                task_id=f'{identifier}-schedule-job',
                job_template_file="{{ ti.xcom_pull(task_ids='template-generator', "
                + "key='job-file') }}",
                backoff_limit=5,
            ...
            clean_up = PythonOperator(
                task_id='clean-up-template', python_callable=helpers.clean_up_templates
            )
    ...
    def create_stats_dag(
        *,
        ...
        with DAG(
            ...
            k8_job = KubernetesJobOperator(
                task_id=f'{identifier}-stats-job',
                job_template_file="{{ ti.xcom_pull(task_ids='template-generator', "
                + "key='job-file') }}",
                backoff_limit=5,
                wait_until_job_complete=True,

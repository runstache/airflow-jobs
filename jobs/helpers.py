...
def create_job_spec(
    secret_name: str, job_name: str, image: str, commands: list[str], base_url: str
) -> dict:
    """
...
def publish_template(**kwargs):
    ...
    job_spec = create_job_spec(
        Variable.get(kwargs['secret_name']),
        kwargs['job_name'],
    ...
    task_instance.xcom_push(key='job-file', value=os.path.join(output_path, file_name))
